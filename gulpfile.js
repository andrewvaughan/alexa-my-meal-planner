...
const allFiles = ['gulpfile.js', 'src/**/*.js', 'test/**/*.spec.js'],
      sourceFiles = ['src/**/*.js'],
...
gulp.task('changelog', () => {
  ...
  const getPrevTag = (err, stdout) => {
    ...
    plugin.git.exec({
      args : `log ${tagString} --no-merges --reverse --pretty=format:'- [view](https://github.com/andrewvaughan/alexa-my-meal-planner/commit/%H) &bull; %s'`   /* eslint max-len: 0 */
    }, printChangeLog);
